...
def create_or_update_weekly_report(db: Session, data) -> dict:
    existing = db.query(WeeklyReport).filter(
        WeeklyReport.week_id == data.week_id,
    ...
    if existing:
        ...
        db.commit()
        db.refresh(existing)
        return {"id": existing.id}, "周报更新成功"
    else:
        report = WeeklyReport(**data.model_dump())
        db.add(report)
        db.commit()
        db.refresh(report)
        return {"id": report.id}, "周报创建成功"


def get_weekly_report_by_id(db: Session, report_id: int) -> dict:
    report = db.query(WeeklyReport).filter(WeeklyReport.id == report_id).first()
    if not report:
        raise ResourceNotFoundException("周报", report_id)

    result = _report_to_dict_full(report)
...
def update_weekly_report(db: Session, report_id: int, data) -> dict:
    report = db.query(WeeklyReport).filter(WeeklyReport.id == report_id).first()
    if not report:
        raise ResourceNotFoundException("周报", report_id)
...
def analyze_weekly_report(db: Session, report_id: int) -> dict:
    report = db.query(WeeklyReport).filter(WeeklyReport.id == report_id).first()
    if not report:
        raise ResourceNotFoundException("周报", report_id)
...
def get_team_summary(db: Session, week_id: Optional[int] = None) -> dict:
    if not week_id:
...
def _report_to_dict_full(r: WeeklyReport) -> dict:
    return {
        "id": r.id,
        "week_id": r.week_id,
        "week_name": r.week.name if r.week else None,
